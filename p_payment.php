...
$installments = [];
$payment_message = "";
$loan_type = ""; // Will be 'business_loan', 'micro_loan', 'lease_loan' or 'interest_only'
$success_data = [];
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['search'])) {
    ...
    $query = "SELECT la.id AS loan_id, la.loan_amount, m.full_name, m.nic, 
                     CASE WHEN lp.interest_type = 'interest_only' THEN 'interest_only' ELSE 'business_loan' END AS loan_type
    ...
    if (!empty($loan_details)) {
        // For interest-only loans, calculate accrued interest
        if ($loan_type == 'interest_only') {
            // Get total accrued interest from daily_interest_accruals table
            $interest_query = $conn->prepare("SELECT SUM(amount) AS total_interest 
                                            FROM daily_interest_accruals 
                                            WHERE loan_id = ?");
            $interest_query->bind_param("s", $loan_details['loan_id']);
            $interest_query->execute();
            $interest_result = $interest_query->get_result()->fetch_assoc();
            
            // Get any payments already made
            $paid_query = $conn->prepare("SELECT SUM(interest_paid) AS total_paid 
                                        FROM payments 
                                        WHERE loan_id = ?");
            $paid_query->bind_param("s", $loan_details['loan_id']);
            $paid_query->execute();
            $paid_result = $paid_query->get_result()->fetch_assoc();
            
            $total_interest = $interest_result['total_interest'] ?? 0;
            $total_paid = $paid_result['total_paid'] ?? 0;
            
            $loan_details['total_due'] = $total_interest - $total_paid;
            $loan_details['interest_due'] = $loan_details['total_due'];
            $loan_details['capital_due'] = 0;
            
            // For display purposes, create a dummy installment
            $installments = [[
                'installment_date' => date('Y-m-d'),
                'installment_amount' => 0,
                'paid_amount' => $total_paid,
                'interest_due' => $loan_details['interest_due'],
                'capital_due' => 0,
                'total_due' => $loan_details['total_due'],
                'status' => $loan_details['total_due'] > 0 ? 'pending' : 'paid'
            ]];
            
        } else {
            // Regular loan processing for other types
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['pay'])) {
    ...
    try {
        if ($loan_type == 'interest_only') {
        // Special handling for interest-only loans
        
        // Get total accrued interest and paid interest
        $interest_query = $conn->prepare("SELECT SUM(amount) AS total_interest 
                                        FROM daily_interest_accruals 
                                        WHERE loan_id = ?");
        $interest_query->bind_param("s", $loan_id);
        $interest_query->execute();
        $interest_result = $interest_query->get_result()->fetch_assoc();
        ...
        $paid_query = $conn->prepare("SELECT SUM(interest_paid) AS total_paid 
                                    FROM payments 
                                    WHERE loan_id = ?");
        $paid_query->bind_param("s", $loan_id);
        $paid_query->execute();
        $paid_result = $paid_query->get_result()->fetch_assoc();
        
        $total_interest = $interest_result['total_interest'] ?? 0;
        $total_paid = $paid_result['total_paid'] ?? 0;
        $interest_due = $total_interest - $total_paid;
        
        // Calculate how much to apply to interest and capital
        $interest_paid = min($paid_amount, $interest_due);
        $capital_paid = max(0, $paid_amount - $interest_due);
        
        // Get loan details to verify capital payment doesn't exceed principal
        $loan_query = $conn->prepare("SELECT la.loan_amount, 
                                    IFNULL(SUM(p.capital_paid), 0) AS total_capital_paid
                                    FROM loan_applications la
                                    LEFT JOIN payments p ON la.id = p.loan_id
                                    WHERE la.id = ?");
        $loan_query->bind_param("s", $loan_id);
        $loan_query->execute();
        $loan_data = $loan_query->get_result()->fetch_assoc();
        
        $remaining_principal = $loan_data['loan_amount'] - $loan_data['total_capital_paid'];
        
        if ($capital_paid > $remaining_principal) {
            throw new Exception("Capital payment cannot exceed remaining principal of Rs " . 
                            number_format($remaining_principal, 2));
        }
        
        // Create payment record
        $payment_stmt = $conn->prepare("INSERT INTO payments 
                                    (loan_id, amount, interest_paid, capital_paid, payment_date) 
                                    VALUES (?, ?, ?, ?, ?)");
        $payment_stmt->bind_param("sdds", $loan_id, $paid_amount, $interest_paid, $capital_paid, $payment_date);
        $payment_stmt->execute();
        
        // If capital was paid, update the loan's principal
        if ($capital_paid > 0) {
            $update_loan = $conn->prepare("UPDATE loan_applications 
                                        SET loan_amount = loan_amount - ? 
                                        WHERE id = ?");
            $update_loan->bind_param("ds", $capital_paid, $loan_id);
            $update_loan->execute();
        }
    } else {
            // Regular payment processing for other loan types
        ...
        <?php if (!empty($loan_details)): ?>
            ...
            <div class="card mb-4">
                <div class="card-header bg-primary text-white">
                    Loan Details
                    <span class="badge <?= $loan_type == 'business_loan' ? 'bg-info' : ($loan_type == 'micro_loan' ? 'bg-warning' : ($loan_type == 'leasing' ? 'bg-secondary' : 'interest-only-badge')) ?> loan-type-badge float-end">
                ...
                <div class="card-body">
                    <div class="row">
                        ...
                        <div class="col-md-6">
                            ...
                            <?php if ($loan_type == 'interest_only'): ?>
            ...
            <!-- Installments Table -->
            <h4><?= $loan_type == 'interest_only' ? 'Interest Accrual' : 'Installments' ?></h4>
