...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $loan_type = ($loan_amount <= 40000) ? 'ML' : (($loan_amount <= 100000) ? 'LL' : 'BL'); // Example threshold
    ...
    $query = "INSERT INTO lease_applications (id, branch, credit_officer, member_id, 
              guarantor1_name, guarantor1_nic, guarantor1_mobile, 
              guarantor2_name, guarantor2_nic, guarantor2_mobile, vehicle_id,
              loan_product_id, loan_amount, installments, interest_rate, rental_value, status) 
              VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
    
    $stmt = $conn->prepare($query);
    $stmt->bind_param("siiisssssssiiddds", $loan_id, $branch, $credit_officer, $member_id, 
                    $guarantor1_name, $guarantor1_nic, $guarantor1_mobile, 
                    $guarantor2_name, $guarantor2_nic, $guarantor2_mobile, $vehicle_id,
    ...
    if ($stmt->execute()) {
        echo "<script>alert('✅ Loan Application Submitted Successfully! ID: $loan_id'); window.location.href='leasing.php';</script>";
    } else {
    ...
    <div class="card loan-application-card shadow">
        ...
        <div class="card-body">
            <form method="POST" class="needs-validation" novalidate>
                ...
                <div class="form-section mb-4">
                    <h4 class="section-title"><i class="fas fa-car me-2"></i>Vehicle Information</h4>
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            ...
                            <select name="vehicle_id" id="vehicle_id" class="form-select" required>
                                ...
                                $vehicles = $conn->query("SELECT id, vehicle_no, make, model FROM vehicles ORDER BY vehicle_no");
                                
                                // Display each vehicle as an option
                                while ($vehicle = $vehicles->fetch_assoc()) {
                                    $displayText = htmlspecialchars("{$vehicle['vehicle_no']} - {$vehicle['make']} {$vehicle['model']}");
                                    echo "<option value='{$vehicle['id']}'>$displayText</option>";
                                }
                                ?>
                            </select>
                        </div>
