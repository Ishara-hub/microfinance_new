...
if (isset($_POST['approve'])) {
    ...
    $loanQuery = "SELECT la.*, m.full_name, lp.name AS product_name 
    ...
    if ($updateStmt->execute()) {
        ...
        header("Location: loan_disburse.php");
        exit();
    }
}
...
if (isset($_POST['reject'])) {
    ...
    $updateQuery = "UPDATE loan_applications SET status = 'rejected', reject_reason = ? WHERE id = ?";
    $updateStmt = $conn->prepare($updateQuery);
...
$result = $conn->query("SELECT la.id, m.full_name, lp.name, la.loan_amount, 
                        la.installments, la.created_at, la.credit_officer
                        FROM loan_applications la
                        JOIN members m ON la.member_id = m.id
                        JOIN loan_products lp ON la.loan_product_id = lp.id
