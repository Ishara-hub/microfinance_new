...
if (isset($_SESSION['payment_success'])) {
    ...
    if (is_string($_SESSION['payment_success'])) {
        $success_data = json_decode($_SESSION['payment_success'], true);
    } else {
        $success_data = $_SESSION['payment_success'];
    }
    
    if (is_array($success_data) && isset($success_data['message'])) {
        $payment_message = $success_data['message'];
        unset($_SESSION['payment_success']); // Clear after displaying
        ...
        if (!isset($_POST['search_term']) && isset($success_data['loan_id'])) {
            ...
            $_POST['search'] = true; // Trigger search
        }
    }
}
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['search'])) {
    ...
    if ($result->num_rows > 0) {
        ...
        // If not found in regular loans, try micro loans
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['pay'])) {
    ...
    try {
        ...
        // Store success data in session as JSON
        ...
        $conn->commit();
        header("Location: pro_pay_m_new.php");
        exit();
        ...
        <?php if (!empty($loan_details)): ?>
            ...
            <div class="card mb-4">
                <div class="card-header bg-primary text-white">
                    Loan Details
                    <span class="badge <?= $loan_type == 'business_loan' ? 'bg-info' : 'bg-warning' ?> loan-type-badge float-end">
                        <?= strtoupper($loan_type) ?> LOAN
                    </span>
                </div>
                <div class="card-body">
                    <div class="row">
